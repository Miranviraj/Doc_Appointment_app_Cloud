...
app.post('/api/book-appointment', verifyToken, async (req, res) => {
  try {
    ...
    if (isNaN(appointmentDateObj.getTime())) { // Use .getTime() for a robust check
...
app.delete('/api/appointment/:id', verifyToken, async (req, res) => {
  try {
    ...
    if (appointment.patientId !== userId) {
      return res.status(403).send('Forbidden: You cannot cancel this appointment.');
    }

    // 2. Logical Constraint: Is the appointment in the past?
    const now = admin.firestore.Timestamp.now();
    if (appointment.appointmentTime < now) {
      return res.status(400).send('You cannot cancel an appointment that is already in the past.');
    }
